...
export const StripeIntegrationScreen: React.FC<ScreenProps> = ({
  ...
  const [publicKey, setPublicKey] = useState('pk_live_51Mv8X3LkdG3D0X7J9Z2a8b3c...');
  const [webhookSecret, setWebhookSecret] = useState('whsec_9d04f8e2c31b5a92d84a7e93c1...');
    ...
    <div className="w-full bg-card text-foreground font-sans antialiased text-xs">
      ...
      <div className="w-full bg-card border-b border-border p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2 tabular-nums text-[10px] text-muted-foreground">
            ...
            <span className="text-foreground font-bold">REVENUE &amp; STRIPE MERCHANT</span>
            <span className="text-foreground font-bold">&gt;&gt;</span>
            <span className="bg-primary text-primary-foreground px-1 text-[9px] font-bold">API GATEWAY</span>
          </div>
          <div className="flex items-baseline gap-2">
            <h1 className="font-bold text-lg md:text-xl tracking-tight uppercase text-foreground">STRIPE CONNECTED GATEWAY CONFIG</h1>
        ...
        <div className="xl:col-span-7 space-y-6">
          <div className="border-2 border-border p-5 bg-card shadow-card-md">
            <div className="flex items-center justify-between border-b border-border pb-3 mb-4 tabular-nums">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-sm uppercase text-foreground">SECTION A: API CREDENTIALS &amp; ENVIRONMENT MODULATION</h3>
              </div>
              <div className="flex border border-border overflow-hidden tabular-nums text-[10px]">
                <button 
                  ...
                  className={`px-2.5 py-0.5 font-bold uppercase transition-colors cursor-pointer ${stripeMode === 'live' ? 'bg-primary text-primary-foreground' : 'bg-card text-foreground'}`}
                >
                  LIVE
                </button>
                <button 
                  onClick={() => { setStripeMode('test'); showToast('GATEWAY ROUTED TO SANDBOX'); }}
                  className={`px-2.5 py-0.5 font-bold uppercase transition-colors cursor-pointer ${stripeMode === 'test' ? 'bg-primary text-primary-foreground' : 'bg-card text-foreground'}`}
                >
                  TEST
                </button>
              </div>
            ...
            <div className="space-y-4 tabular-nums text-xs">
              <div className="space-y-1.5">
                <label className="block text-[10px] text-muted-foreground uppercase font-bold">STRIPE PUBLISHABLE KEY</label>
              ...
              <div className="space-y-1.5">
                <label className="block text-[10px] text-muted-foreground uppercase font-bold">STRIPE SECRET API KEY</label>
                <div className="flex items-center">
                  <input 
                    type="password" 
                    value="••••••••••••••••••••••••••••••••••••••••••••••••" 
                ...
                <span className="text-[10px] text-muted-foreground block">Stripe secret keys are restricted to server-side environments. To modify, populate <code className="bg-muted/40 px-1 font-bold text-foreground">STRIPE_SECRET_KEY</code> in the environment console.</span>
              </div>
            </div>
          </div>
          ...
          <div className="border-2 border-border p-5 bg-card shadow-card-md">
            <div className="flex items-center justify-between border-b border-border pb-3 mb-4">
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 bg-black"></span>
                <h3 className="font-bold text-sm uppercase tabular-nums tracking-wider">SECTION B: REALTIME EVENT WEBHOOK HANDLERS</h3>
              </div>
            ...
            <div className="space-y-4 tabular-nums text-xs">
              <div className="space-y-1.5">
                <label className="block text-[10px] text-muted-foreground uppercase font-bold">WEBHOOK LISTENER ENDPOINT</label>
                <div className="flex items-center">
                  <input 
                    type="text" 
              ...
              <div className="space-y-1.5">
                <label className="block text-[10px] text-muted-foreground uppercase font-bold">WEBHOOK SIGNING SECRET (WHSEC)</label>
                <div className="flex items-center">
                  <input 
                    type="text" 
                    value={webhookSecret} 
                ...
                <span className="text-[10px] text-muted-foreground block">Required to verify that inbound events originate exclusively from Stripe.com. Mutates <code className="bg-muted/40 px-1 font-bold text-foreground">STRIPE_SIGNING_SECRET</code>.</span>
              </div>

              <div className="border border-border p-3 bg-muted/30 space-y-1.5">
                <span className="text-[10px] text-muted-foreground uppercase font-bold block">LISTENED EVENTS STATUS</span>
                <div className="flex justify-between items-center text-xs tabular-nums">
                  <span>↳ <code className="bg-muted/40 px-1 font-bold text-foreground">payment_intent.succeeded</code>:</span>
                  <span className="bg-primary text-primary-foreground text-[9px] px-1.5 py-0.2 font-bold uppercase">HANDLED</span>
                </div>
                <div className="flex justify-between items-center text-xs tabular-nums">
                  <span>↳ <code className="bg-muted/40 px-1 font-bold text-foreground">charge.refunded</code>:</span>
                  <span className="bg-primary text-primary-foreground text-[9px] px-1.5 py-0.2 font-bold uppercase">HANDLED</span>
                </div>
              </div>
            </div>
          ...
          <div className="border-2 border-border p-4 bg-card shadow-card-md">
            ...
            <div className="space-y-3 tabular-nums text-xs">
              <div className="border border-border p-3 bg-muted/30 space-y-1">
                <span className="text-[9px] text-muted-foreground uppercase block font-bold">MERCHANT ACCOUNT DESIGNATION</span>
                <div className="font-bold text-sm text-foreground">All About Pawz Holdings LLC</div>
                <span className="text-[10px] text-muted-foreground block">Stripe ID: acct_1Mv8X3LkdG3D0X7J</span>
              </div>

              <div className="border border-border p-3 bg-muted/30 space-y-1">
                <span className="text-[9px] text-muted-foreground uppercase block font-bold">DIRECT DEPOSIT BANK ROUTING</span>
                <div className="font-bold text-sm text-foreground">CHASE COMMERCIAL ******9812</div>
            ...
            <div className="mt-4 pt-3 border-t border-border">
              <a 
                ...
                target="_blank" 
                rel="noreferrer"
                className="w-full bg-primary text-primary-foreground py-2 text-xs tabular-nums uppercase font-bold hover:bg-muted flex items-center justify-center gap-1.5 cursor-pointer"
              >
      ...
      <div className="w-full bg-muted/30 border-t border-border border-b border-border p-4 flex flex-col md:flex-row items-center justify-between gap-4 select-none tabular-nums text-xs">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="border border-border bg-card px-2 py-0.5 text-foreground font-bold tabular-nums">STR_REF #TX-48210-9004</span>
          <button 
